...
local function dimmerCalculateCurrentLevel(state)
    local _, startLevel, targetLevel, startTime, transitionTime = table.unpack(state, 1, 5)
    ...
    local delta = (targetLevel - startLevel) * elapsed_ms // transitionTime_ms
...
local function dimmerLevelTransition(device, dimmer, onOffAttributeChanged)
    ...
    local state, onOffAttrs, levelAttrs = table.unpack(dimmer, 1, 3)
    local command, startLevel, targetLevel, startTime, transitionTime, timer = table.unpack(state, 1, 6)
    local transitionTime_ms = transitionTime * 100
    local elapsed_ms = os.msticks() - startTime
    if elapsed_ms > transitionTime_ms then
        elapsed_ms = transitionTime_ms
    ...
    local remainingTime = remaining_ms // 100
    ...
    if transitionTime_ms > 0 then
        delta = delta * elapsed_ms // transitionTime_ms
    end
...
local function dimmerCallback(device, ref, action, data, tag)
    ...
    if action == 2 or action == 3 then
        ...
        for ____, clusterReq in ipairs(req) do
            local clusterId, clusterAttrs = table.unpack(clusterReq, 1, 2)
    ...
    elseif action == 6 then
        ...
        for ____, clusterReq in ipairs(req) do
            ...
            local cluster, attributes, nullAttributes = table.unpack(clusterReq, 1, 3)
    ...
    elseif action == 8 then
        local req = data
        local clusterId, commandId, commandData = table.unpack(req, 1, 3)
        if clusterId == 6 then
            local target = nil
            if commandId == 1 then
                target = true
