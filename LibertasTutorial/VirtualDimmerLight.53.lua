...
local function dimmerCalculateCurrentLevel(state)
    local _, startLevel, targetLevel, startTime, transitionTime = table.unpack(state)
...
local function dimmerLevelTransition(device, dimmer, onOffAttributeChanged)
    ...
    local state, onOffAttrs, levelAttrs = table.unpack(dimmer)
    local command, startLevel, targetLevel, startTime, transitionTime, timer = table.unpack(state)
    local transitionTime_ms = transitionTime * 100
    local elapsed_ms = os.msticks() - startTime
    if elapsed_ms > transitionTime_ms then
        elapsed_ms = transitionTime_ms
...
local function dimmerCallback(device, ref, action, data, tag)
    ...
    elseif action == 6 then
        ...
        for ____, clusterReq in ipairs(req) do
            ...
            local cluster, attributes, nullAttributes = table.unpack(clusterReq)
    ...
    elseif action == 8 then
        local req = data
        local clusterId, commandId, commandData = table.unpack(req)
        if clusterId == 6 then
            local target = nil
            if commandId == 1 then
                target = true
